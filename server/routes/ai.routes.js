...
async function getBusinessContext(prisma) {
  ...
  // Get outstanding payables (at supplier level)
  ...
  purchases.forEach(p => {
    if (!supplierPurchaseTotals[p.supplier_id]) {
      supplierPurchaseTotals[p.supplier_id] = 0;
    }
    supplierPurchaseTotals[p.supplier_id] += parseFloat(p.total_amount || 0);
  });

  // Calculate total payables
  let totalPayables = 0;
  Object.keys(supplierPurchaseTotals).forEach(supplierId => {
    const totalPurchases = supplierPurchaseTotals[supplierId];
...
router.post('/chat', async (req, res) => {
  try {
    ...
    if (!GROQ_API_KEY) {
      return res.status(500).json({ success: false, message: 'AI service not configured. Add GROQ_API_KEY to .env' });
    }
    ...
    const history = await req.prisma.chatHistory.findMany({
    ...
    await req.prisma.chatHistory.createMany({
      data: [
        { user_id: req.user.user_id, role: 'user', content: message },
        { user_id: req.user.user_id, role: 'assistant', content: aiResponse }
      ]
    });
