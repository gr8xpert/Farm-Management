...
router.get('/outstanding', async (req, res) => {
  try {
    ...
    purchases.forEach(p => {
      const totalPaid = p.payments.reduce((sum, pay) => sum + parseFloat(pay.payment_amount), 0);
      const outstanding = parseFloat(p.total_amount || 0) - totalPaid;
      if (outstanding > 0) {
        const daysOutstanding = Math.floor((new Date() - new Date(p.po_date)) / (1000 * 60 * 60 * 24));
