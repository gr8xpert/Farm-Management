...
export default function SaleReturnForm() {
  ...
  const fetchSales = async () => {
    try {
      const response = await api.get('/sales', { params: { limit: 100 } })
  ...
  const handleSaleChange = async (saleId) => {
    ...
    if (!saleId) { setSaleDetails(null); return }

    setLoading(true)
    try {
      const response = await api.get(`/sale-returns/sale/${saleId}/details`)
  ...
  const handleSubmit = async (e) => {
    e.preventDefault()
    if (!selectedSale) { toast.error('Please select a sale'); return }
    ...
    try {
      await api.post('/sale-returns', {
        sale_id: parseInt(selectedSale),
        sr_date: formData.sr_date,
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        <button onClick={() => navigate('/sale-returns')} className="p-2 rounded-lg hover:bg-gray-100">
          <ArrowLeft className="w-5 h-5" />
        </button>
        <h1 className="text-2xl font-bold text-gray-900">New Sale Return</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="card">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <div>
              ...
              <select value={selectedSale} onChange={(e) => handleSaleChange(e.target.value)} className="input-field" required>
                <option value="">Select Sale</option>
                {sales.map(s => <option key={s.sale_id} value={s.sale_id}>Sale #{s.sale_id} - {s.customer?.customer_name}</option>)}
              </select>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-1">Return Date *</label>
              <input type="date" value={formData.sr_date} onChange={(e) => setFormData({ ...formData, sr_date: e.target.value })} className="input-field" required />
            </div>
            <div>
        ...
        <div className="flex justify-end gap-3">
          <button type="button" onClick={() => navigate('/sale-returns')} className="btn-secondary">Cancel</button>
          <button type="submit" disabled={submitting || !selectedSale} className="btn-primary flex items-center gap-2">
