...
export default function PurchaseReturns() {
  ...
  const columns = [
    { key: 'pr_no', label: 'Return #' },
    { key: 'pr_date', label: 'Date', render: (val) => val ? new Date(val).toLocaleDateString() : '-' },
    { key: 'purchase', label: 'PO #', render: (val) => val?.po_no || '-' },
  ...
  const fetchReturns = async (page = 1) => {
    setLoading(true)
    try {
      const response = await api.get('/purchase-returns', { params: { page, limit: 10 } })
  ...
  const handleDelete = async () => {
    setSubmitting(true)
    try {
      await api.delete(`/purchase-returns/${deletingItem.pr_no}`)
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-semibold text-gray-800">Purchase Returns</h1>
        <button
          ...
          <div className="flex min-h-screen items-center justify-center p-4">
            ...
            <div className="relative w-full max-w-2xl modal-content animate-fadeIn">
              <div className="px-5 py-4 border-b border-gray-100 flex justify-between items-center">
                <h3 className="text-base font-semibold text-gray-800">Purchase Return #{viewingReturn.pr_no}</h3>
                <button
                  onClick={() => setViewingReturn(null)}
              ...
              <div className="p-5 space-y-4">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><span className="text-gray-500">Date:</span> <span className="text-gray-800">{new Date(viewingReturn.pr_date).toLocaleDateString()}</span></div>
                  <div><span className="text-gray-500">Original PO:</span> <span className="text-gray-800">#{viewingReturn.purchase?.po_no}</span></div>
                  <div><span className="text-gray-500">Supplier:</span> <span className="text-gray-800">{viewingReturn.supplier?.supplier_name}</span></div>
      ...
      <DeleteConfirm
        ...
        title="Delete Return"
        message={`Delete Return #${deletingItem?.pr_no}? This cannot be undone.`}
        loading={submitting}
      />
    </div>
  )
