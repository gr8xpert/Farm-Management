...
export default function OutstandingReport() {
  ...
  const exportCSV = () => {
    ...
    const headers = activeTab === 'receivables'
      ...
      : ['Supplier', 'Purchases', 'Total', 'Paid', 'Outstanding', 'Days']

    const rows = items.map(i => activeTab === 'receivables'
      ? [i.customer?.customer_name, i.sale_id, formatDate(i.sale_date), i.total, i.paid, i.outstanding, i.daysOutstanding]
      : [i.supplier?.supplier_name, i.purchaseCount, i.total, i.paid, i.outstanding, i.daysOutstanding]
    )
              ...
              <table className="w-full">
                <thead>
                  <tr className="bg-gray-50 border-b border-gray-200">
                    ...
                    <th className="text-left py-3 px-4 text-xs font-semibold text-gray-600 uppercase">
                      {activeTab === 'receivables' ? 'Sale #' : 'Purchases'}
                    </th>
                    <th className="text-left py-3 px-4 text-xs font-semibold text-gray-600 uppercase">{activeTab === 'receivables' ? 'Date' : 'Oldest'}</th>
                    ...
                    (activeTab === 'receivables' ? data.receivables : data.payables)?.map((item, idx) => (
                      <tr key={idx} className="border-b border-gray-100 hover:bg-gray-50/50">
                        ...
                        <td className="py-3 px-4 text-sm text-gray-600">
                          {activeTab === 'receivables' ? `#${item.sale_id}` : `${item.purchaseCount} POs`}
                        </td>
                        <td className="py-3 px-4 text-sm text-gray-600">
                          {activeTab === 'receivables' ? formatDate(item.sale_date) : `${item.daysOutstanding} days ago`}
                        </td>
